...
class LRUStrategy: public Poco::AbstractStrategy<TKey, TValue>
...
public:
	LRUStrategy(std::size_t size): 
		_size(size)
	{
		if (_size < 1) throw Poco::InvalidArgumentException("size must be > 0");
	}
    ...
    void onAdd(const void*, const Poco::KeyValueArgs <TKey, TValue>& args)
	{
	...
	void onClear(const void*, const Poco::EventArgs& args)
	{
		_keys.clear();
		_keyIndex.clear();
	}

	void onIsValid(const void*, Poco::ValidArgs<TKey>& args)
	{
