...
{
public:
	Poco::FIFOEvent<const Poco::KeyValueArgs<TKey, TValue >, TEventMutex > Add;
	Poco::FIFOEvent<const Poco::KeyValueArgs<TKey, TValue >, TEventMutex > Update;
	Poco::FIFOEvent<const TKey, TEventMutex>                         Remove;
	Poco::FIFOEvent<const TKey, TEventMutex>                         Get;
	Poco::FIFOEvent<const Poco::EventArgs, TEventMutex>                    Clear;
	...
	void update(const TKey& key, const TValue& val)
		/// Adds the key value pair to the cache. Note that adding a NULL std::shared_ptr will fail!
		/// If for the key already an entry exists, it will be overwritten.
    ...
    std::shared_ptr<TValue> get(const TKey& key)
		/// Returns a std::shared_ptr of the value. The SharedPointer will remain valid
		/// even when cache replacement removes the element.
		/// If for the key no value exists, an empty std::shared_ptr is returned.
	{
		typename TMutex::ScopedLock lock(_mutex);
		return doGet (key);
	}
...
protected:
	mutable Poco::FIFOEvent<Poco::ValidArgs<TKey> > IsValid;
	mutable Poco::FIFOEvent<KeySet>           Replace;
	...
	std::shared_ptr<TValue> doGet(const TKey& key)
		/// Returns a std::shared_ptr of the cache entry, returns 0 if for
